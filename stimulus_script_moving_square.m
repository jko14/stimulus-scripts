function stimulus_script_moving_square(Q, dur, direction, period, speed, lum, cont)
%STIMULUS_SCRIPT_MOVING_SQUARE Displays a moving square wave in the stimulus rect.
%   For 'dur' seconds: displays a square wave, whose period is 'period'
...
while elapsed < dur
    ...
    color = mod(xy(1, :) - offset, period_pix);
    color = (color < period_pix / 2);
    % Transform to [-1 1]
    color = color * 2 - 1;
    color = lum * (1 + color * cont);
